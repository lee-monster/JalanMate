...
> - **브랜드 팔레트** (로고 + 인니·말련·싱가포르 국기색에서 도출): 레드 `#E11D2E`
>   (primary — 세 국기 공통색 + 로고), 네이비 `#1A3C8C` (secondary — 말레이시아
>   국기 청색), 골드 `#F5B301` (accent/별점 — 말레이시아 국기 금색), 화이트(베이스 —
>   세 국기 공통). CSS 토큰은 `css/travel-app.css`의 `:root`
>   (`--t-primary` / `--t-navy` / `--t-accent`).
>
